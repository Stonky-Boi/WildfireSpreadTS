...
class PhysicalAttentionMambaModel(BaseModel):
    """
    Level 8: Physical Attention Masking.
    
    Integrates Topographic Constraint Attention and Fire-Front Focal Attention.
    Instead of modifying the internal Swin Softmax (which breaks CUDA optimizations), 
    we apply these physical priors as multiplicative spatial gates before the transformer.
    """
    ...
    def __init__(
        ...
        loss_function: str = "PhysicsAware",
        use_doy: bool = False,
        ...
        **kwargs: Any
        ...
        # --- Level 8: Physical Attention Masking Modules ---
        
        # Topographic Reachability Score R(i,j): sigma(Conv(E_terrain))
        self.reachability_net = nn.Sequential(
            nn.Conv2d(hidden_dim, 1, kernel_size=3, padding=1),
            nn.Sigmoid()
        )
        
        # Fire-Front Focal Weight (eta)
    ...
    def _get_fire_boundary(self, fire_mask: torch.Tensor) -> torch.Tensor:
        """
        Extracts boundary B_t using morphological operations.
        B_t = (Fire ⊕ S) - (Fire ⊖ S)
        """
        # B_t mask isolates the fire front
        dilation = F.max_pool2d(fire_mask, kernel_size=3, stride=1, padding=1)
        erosion = -F.max_pool2d(-fire_mask, kernel_size=3, stride=1, padding=1)
    ...
    def forward(self, x: torch.Tensor, doys: Any = None) -> torch.Tensor:
        ...
        # --- Stage 3: Physical Attention Masking ---
        
        # 1. Compute Topographic Reachability Mask R(i', j')
        reachability_mask = self.reachability_net(e_terrain)
        
        # 2. Compute Fire-Front Focal Mask B_t(i', j')
        current_fire_mask = x_permuted[:, self.fire_index, -1, :, :].float()
        boundary_mask = self._get_fire_boundary(current_fire_mask)
        
        # 3. Modulate fused features before Swin Reasoning
        focal_bias = 1.0 + (self.eta * boundary_mask)
        f_masked = (e_fused * focal_bias) * reachability_mask
        
        # Swin Backbone
        x0 = self.swin_features[0](f_masked)
